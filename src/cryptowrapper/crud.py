...
async def get_coins_by_ids(db: AsyncIOMotorDatabase, coins_ids: list[str]) -> list[dict]:
...
async def add_coin_id_for_user_coin(db: AsyncIOMotorDatabase, coins: list[dict], user_id: str | int) -> dict:
    observed_coins = [c["_id"] for c in coins]
    await db.accounts.find_one_and_update({"user_id": str(user_id)},
        {"$push": {"_id": {"$each": observed_coins}}})
